...
export const checkDuplicate = async (email: string, phone: string): Promise<{ isDuplicate: boolean; field?: string }> => {
  try {
...
export const addContact = async (contactData: ContactData): Promise<ContactResponse> => {
  try {
    const sheets = getSheetsClient();
    const spreadsheetId = getSpreadsheetId();
...
export const initializeSpreadsheet = async (): Promise<void> => {
  try {
    const sheets = getSheetsClient();
    const spreadsheetId = getSpreadsheetId();
